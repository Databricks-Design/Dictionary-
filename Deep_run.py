...
def main():
    ...
    gc.collect()
    tracemalloc.start()
    # Capture 25 frames of stack trace to see exactly who called whom
        ...
        for i in range(0, len(df), BATCH_SIZE):
            ...
            if SAVE_OUTPUTS:
                ...
                for rr in raw_results:
                    ...
                    if hasattr(rr, 'output_tensors'):
                        tensors = rr.output_tensors() if callable(rr.output_tensors) else rr.output_tensors
                    else: tensors = []

                    t_map = {}
                    for t in tensors:
                        name = t.name() if callable(getattr(t, 'name', None)) else getattr(t, 'name', 'unknown')
                        t_map[name] = t.as_numpy()

                    labels = t_map.get('label', np.array([])).tolist()
                    texts = t_map.get('extractedText', np.array([])).tolist()
                    ids = t_map.get('entityId', np.array([])).tolist()
    ...
    report.append("=" * 100)
    report.append(f"{'GROWTH (MB)':<12} | {'COUNT':<8} | {'FILE LOCATION'}")
    report.append("-" * 100)
    
    total_explained = 0.0
    
    # Show Top 50 Offenders Globally
    for stat in top_stats[:50]:
        ...
        if growth_mb > 0.001: 
            ...
            # Keep full path so you can see exactly which library it is
            report.append(f"{growth_mb:<+12.4f} | {stat.count_diff:<8} | {line_desc}")
